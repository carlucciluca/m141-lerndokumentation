...
1. **Attribute indexieren:**  
   Alle Attribute indexieren die bei ‘WHERE, ORDER BY, GROUP BY’ zum Einsatz kommen.

2. **UNIONS verwenden:**  
   Wenn in einer WHERE-Bedienung mit “or” gearbeitet wird, UNIONS (Kombiniert das Resultat von mehreren Selects) erstellen.

3. **Wildcards:**  
   Führende Wildcards bei "Selects" vermeiden.

4. **Full-Text-Searches nutzen:**  
   MySQL unterstützt Full-Text-Searches - das ist besser als Wildcard-Queries.
...
1. SQL-Datei mit folgendem Inhalt erstellen:

<details>
<summary>classicmodelsOhneIndex.sql</summary>

```sql
DROP DATABASE IF EXISTS explaintesting;
CREATE DATABASE explaintesting;
USE explaintesting;
...
DROP INDEX `PRIMARY` ON orderdetails;
...
DROP INDEX `customerNumber` ON orders;
...
DROP INDEX `PRIMARY` ON payments;
...
DROP INDEX `PRIMARY` ON productlines;
...
DROP INDEX `PRIMARY` ON products;
...
('S32_1268', '1980s GM Manhattan Express', 'Trucks and Buses', '1:32', 'Motor City Art Classics', 'This 1980s era new look Manhattan express is still active, running from the Bronx to mid-town Manhattan. Has 35 opeining windows and working lights. Needs a battery.', 5099, 53.93, 96.31, 'Active'),
...
(47, 'S32_1268', '1980s GM Manhattan Express New', 55.93),
...
(100, 'S32_1268', '1980s GM Manhattan Express Vintage', 78.93),
...
```bash
mysql -u root -pvagrantM141! < classicmodelsOhneIndex.sql
```

### Schlechte Performance

In den Tabellen der Datenbank sind keine Indexe gesetzt, da diese im SQL-Skript wieder entfernt wurden:

```sql
DROP INDEX `PRIMARY` ON customers;
```

Mit einem EXPLAIN SELECT kann dies veranschaulicht werden:
...
**Ausgabe:**  
Wie man in der Spalte "rows" sieht, gibt es sehr viele Zeilen, welche abgefragt werden.  
Bei diesem Beispiel sind es insgesamt **7 x 110 x 326 x 122 x 2996 = 91’750’822’240** was etwa **91 Mrd.** entspricht.
...
Mit folgenden Befehlen werden die Primary Key Indexe wieder hinzugefügt:
...
Danach wird erneut ein EXPLAIN SELECT ausgeführt:
...
**Ausgabe:**  
Wie man in der Spalte "rows" sieht, gibt es nur noch sehr wenige Zeilen, welche abgefragt werden.  
Nun sind es insgesamt **1 x 1 x 4 x 1 x 1 = 4** was sehr viel weniger ist. Ebenso kann man auch in den Spalten "possible_keys" und "keys" entnehmen, dass diese nun Primary Keys enthalten.
